...
fn enumerate_windows() -> CoreResult<Vec<WindowSnapshot>> {
    ...
    hwnds
        .into_iter()
        .filter_map(snapshot_from_hwnd)
        .filter(WindowSnapshot::is_targetable)
        .collect::<Vec<_>>()
        .into_iter()
        .try_fold(Vec::new(), |mut acc, window| {
            if !acc
                .iter()
...
fn snapshot_from_hwnd(hwnd: HWND) -> Option<WindowSnapshot> {
    ...
    let pid = Pid::new(pid)?;
    let process_name = process::process_name(pid).unwrap_or_else(|| {
