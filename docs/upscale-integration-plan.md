...
`MagpieSourceUploadPlan` prepares DDS `SOURCE` texture uploads from the allocation plan. It records
...
`MagpieWgpuDeclarationPlan` emits the WGSL `@group(0) @binding(...)` resource declarations from the
same pass layout, so HLSL-to-WGSL body translation can reuse the already validated binding map
rather than inventing its own register mapping.
`MagpieWgpuBodyTranslationPlan` is the first body-translation slice. It currently accepts
compute-style one-input/one-output texture copy passes and lowers `OUTPUT[pos] = INPUT[pos]` into
WGSL `textureLoad` / `textureStore` with input and output bounds guards; broader Magpie HLSL syntax
remains explicit follow-up work. It also accepts the simple PS-style sampling shape used by effects
such as Nearest and Bilinear, lowering `return INPUT.SampleLevel(sam, pos, 0)` into guarded
`textureSampleLevel` plus `textureStore` compute WGSL.
`MagpieWgpuShaderPlan` is the compute-pipeline boundary for the comparison path: it accepts
pass-indexed WGSL generated by the future HLSL-to-WGSL translator, validates it against the pass
layout, and creates `wgpu::ShaderModule` plus `wgpu::ComputePipeline` objects.
`MagpieWgpuExecutionPlan` records the ordered compute work once the renderer owns pass-local
pipelines and bind groups. It uses the Magpie-compatible dispatch metadata from `MagpieResourcePlan`
and emits `set_pipeline`, `set_bind_group(0, ...)`, and `dispatch_workgroups(x, y, z)` commands into
a `wgpu::CommandEncoder`.
