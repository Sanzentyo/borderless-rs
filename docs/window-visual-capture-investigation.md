...
- アイコンは `WM_GETICON` / class icon / exe fallback から取得し、alpha 付き PNG としてキャッシュして表示する。`BitmapImage.UriSource` 経路では data URI が表示されなかったため、表示の安定性を優先して `file:///` URI に戻した。
- 画像ファイルキャッシュをなくし、`HICON` や BGRA ピクセルを PNG 化せず直接表示するには、Reactor 側に `WriteableBitmap` / stream / pixel buffer を受け取る image source API を追加する。
